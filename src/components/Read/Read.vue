<template>
  <div ref="dvtop" @click="getshow" class="read" :style="{backgroundColor:getcolor.background}">
    ...
    <div v-if="datashow" class="readbottom">
      <ul class="readbottom-color">
        <li
          ...
          :class="{'mui-icon mui-icon-checkmarkempty':item.background===getcolor.background}"
    ...
    <div class="read-hy" v-show="datahy">
...
import { bookContent, bookMulu, bookSources } from "../../api/api";
...
export default {
  ...
  created() {
    ...
    this.getread(false);
    this.getbookhy(this.getBook._id);
  },
  methods: {
    ...mapMutations({
      setbook: "BOOK_UPDATE",
